 const isOne = (addUserInfo, obj) => {
	if (obj.uid === 1) {
		addUserInfo.call(obj, true)
	} else {
		addUserInfo.call(obj) ;
	}
}

 const isArr = (addUserInfo, objs) => {
	if (Array.isArray(objs)) {
		objs.forEach((obj) => {
			isOne(addUserInfo, obj) ;
		})
	}
}
	...
	if (Array.isArray(articles)) {
		...
		newArticles.forEach((article) => {
			...
			const likeUsers = article.likeUsers ;
			 ...
			 // 添加用户信息到文章
			isOne(addUserInfo, article) ;
			
			// 添加用户信息到评论 
			isArr(addUserInfo, comments) ;
			
			// 添加用户信息到点赞		
			isArr(addUserInfo, likeUsers) ;
		})
	}
...
export const getArticlesByUid = (state, getters) => (uid, user) => {
	...
	if (Array.isArray(articles)) {
		...
		// 使用指定 uid 过滤所有文章
		articles = articles .filter(article => Number.parseInt(uid) === Number.parseInt(article.uid)) ;	
	} else {
		articles = [] ;
	}
...
export const getArticlesByFilter = (state, getters) => (filter) => {
	...
	if (Array.isArray(articles)) {
		...
		switch(filter) {
			case "excellent":
				// 将当前用户的文章设置为精华文章
				filteredArticles = getters.getArticleByUId(1) ;
				break ;
			case "vote":
				// 将赞的最多的文章排在前面
