	...
	if (article) {
		...
		if (articleId === undefined) {
			...
			if (lastArticle) {
				...
				articleId = Number.parseInt(lastArticle.articleId) + 1 ;
			} else {
			 ...
			 // 将当前文章添加到所有文章
			Reflect.set(articles, articles.length, {uid, articleId, title, content, date});
			
		} else {
			// 如果有传 articleId
			for (let article of articles ) {
				// 找到与 articleId 对应的文章
				if (Number.parseInt(article.articleId) === Number.parseInt(articleId)) {
					 // 更新文章的标题
					article.title = title ;
					// 更新文章的内容
		...
		commit("UPDATE_ARTICLES",articles);
		  // 跳转到文章，并附带 articleId 和 showMsg 参数，showMsg 用来指示目标页面显示一个提示，我们稍后添加相关逻辑
...
export const like = ({ commit, state },{articleId, isAdd }) => {
	...
	for (let article of articles) {
		...
		if (Number.parseInt(article.articleId) === Number.parseInt(articleId)) {
			...
			if (isAdd) {
				//是否已赞
				const isAdded = likeUsers.some(likeUsers => Number.parseInt(likeUser.uid) === uid) ;
				
				if (!isAdded) {
					// 在点赞用户列表中加入当前用户
					Reflect.set(likeUsers, likeUsers.length, { uid } );
					//likeUsers.push( { uid });
				}
			} else {
				for (let likeUser of likeUsers) {
					// 找到对应点赞用户时
					if (Number.parseInt(likeUser.uid) === uid) {
						// 删除点赞用户
		...
		if (Number.parseInt(article.articleId) === Number.parseInt(articleId)) {
			...
			if (comment) {
				...
				if (commentId === undefined) {
					...
					//新建 commentId
					if (lastComment) {
						commentId = Number.parseInt(lastComment.commentId) + 1;
					} else {
						commentId = comments.length + 1 ;
					}
					
					// 在评论列表中加入当前评论
					Reflect.set(comments, comments.length, {uid, commentId, content, date} );
				} else {
