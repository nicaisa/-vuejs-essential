<template>
	<div class="navbar-right">
				...
				<ul class="dropdown-menu">
					...
					<li v-if="user">
						<router-link :to="`/${user.name}`">
							<i class="fa fa-list-ul text-md i-middle"></i>个人专栏
						</router-link>
					</li>
					<!-- 编辑资料 -->
					<li>
		...
		<div v-else class="nav navbar-nav github-login">
			<router-link to="/auth/login" class="btn btn-defalut login-btn">
				<i class="fa fa-plus"></i> 登 录
			</router-link>
			<router-link to="/auth/register" class="btn btn-defalut login-btn">
				<i class="fa fa-plus"></i> 注 册
			</router-link>
		</div>
	</div>
</template>
