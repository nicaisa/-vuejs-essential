...
<!-- 	<div class="blog-container"  style="margin-top:20px">
	...
	<div class="col-md-9 left-col pull-right">
		<div class="panel article-body pull-right">
					...
					<div class="voted-users">
						<div class="user-lists"> 
							<span v-for="likeUser in likeUsers">
								...
								<!-- <img :src="user && user.avatar" class="img-thumbnail avatar avatar-middle" :class="{ 'animated swing' : likeUser.uid === 1 }" alt=""> -->
			...
			<!-- 打赏弹窗 -->
			<!-- 使用组件的时候，需要使用 .sync 对 show 进行『双向绑定』： -->
			...
			<div class="replies panel panel-default list-panel replies-index">
				...
				<div class="panel-body">
					<transition-group id="reply-list" name="fade" tag="ul" class="list-group row">
						<li v-for="(comment, index) in comments" :key="comment.commentId" class="list-group media">
			...
			<div id="reply-box" class="reply-box form box-block">
				<div class="form-group comment-editor">
					<textarea v-if="auth" name="" id="editor" cols="30" rows="10"></textarea>
					<textarea v-else disabled  class="form-control" placeholder="需要登录后才能发表评论." style="height:172px" name="" id="" cols="30" rows="10"></textarea>
				</div>
				<div class="form-group reply-post-submit">
					<button id="reply-btn" :disabled="!auth" @click="comment" class="btn btn-primary" type="">
						{{ commentId ? "保存编辑" : "回复" }}
					</button>
					<span v-show="commentId" class="help-inline btn-cancel" ></span>
...
<script>
	...
	export default {
		...
		beforeCreate() {
			...
			if (articleId) {
				ls.setItem('articleId', Number.parseInt(articleId)) ;
			}
		},
		created() {
			//在组件内，我们通过 this.$route 来访问路由对象：
		...
		mounted() {
			 ...
			 if (this.auth) {
				...
				simplemde.codemirror.on("change",() => {
					...
					// 更新 commentHtml,我们先替换原内容中的 emoji标识，然后使用 markdown
		...
		methods: {
			...
			comment() {
				...
				if (this.commentMarkdown && this.commentMarkdown.trim() !== '') {
					...
					if (this.commentId) {
						...
						this.simplemde.value('');// 清空编辑器
						document.querySelector('#reply-btn').focus();// 使回复按钮获得焦点
			...
			renderComments(comments) {
				if (Array.isArray(comments)) {
					...
					//console.log("评论方法",comments,this.comments)
				}
			},
			// 编辑评论
			editComment(commentId, commentIndex) {
				...
				for (const comment of comments) {
					...
					if (Number.parseInt(comment.commentId) === Number.parseInt(commentId)) {
						// 设置编辑器的内容
			...
			// 返回带用户信息的文章的某项属性
			recompute(key) {
				const articeId = this.$route.params.articleId ;
				// 这里的文章是基于 getters.computedArticles 的，所以包含用户信息了
				const article = this.$store.getters.getArticlesById(articeId);
				let arr ;
