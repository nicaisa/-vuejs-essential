...
if (Array.isArray(localArticles) && localArticles.length) {
	nextArticleId = Number.parseInt(localArticles[localArticles.length - 1].articleId) + 1 ;
}
...
export const mockArticles = (num = 10) => {
	...
	const arr = [...Array(num)].forEach((article, index) => {
		...
		while (unames.includes(uname) || forbiddenNames.includes(uname)) {
			uname = Random.first() ;
		}
		
		Reflect.set(unames, unames.length, uname) ;
		
		Reflect.set(articles, articles.length, {
		  uid: index + 2,
	...
	articles.forEach((article, index) => {
		...
		randomArticles.forEach((article, index) => {
			Reflect.set(likeUsers, likeUsers.length, {
				uid: article.uid,
				uname: article.uname
			}) ;
		})
		...
		randomArticles = getRandomArticles(articles) ;
		 ...
		 // 加入评论
		randomArticles.forEach((article, index) => {
			Reflect.set(comments, comments.length, {
				uid: article.uid,
				commentId: index + 1,
				// 随机生成 5 - 10 字的中文文本
